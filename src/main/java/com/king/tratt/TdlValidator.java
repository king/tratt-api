...
class TdlValidator {
	...
	private void validateSequence(Environment env, Sequence sequence, String nodePath) {
		...
		/*
		 * Validate "set" field in all checkPoint first, as the remaining checkpoint 
		 * field validations depends on env.sequenceVariables is set correctly.
		 */
		...
		for (int i = 0; i < checkPoints.size(); i++) {
			...
			if (setters != null) {
				for (int j = 0; j < setters.size(); j++) {
					...
					validateSetter(eventMetaData, s, env, nodePath + "[checkPoints][" + i + "]" + "[set][" + j + "]");
				}
			}
		}
	...
	private void validateCheckPoint(Environment env, CheckPoint checkPoint, String nodePath) {
		...
		try {
			...
			addNodeError("Bad matcher \"" + checkPoint.getMatch() + "\"" + ". Error info: " + e.getMessage(),
					nodePath + "[match]");
		}
	...
	private void validateSetter(EventMetaData eventMetaData, String setExpression, Environment env, String nodePath) {
		try {
