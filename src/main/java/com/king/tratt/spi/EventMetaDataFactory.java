...
public interface EventMetaDataFactory {
	 ...
	 * <p>
	 * If {@code EventName} is not recognized as a known event type, {@code null}
	 * should be returned. Note! Instead of returning {@code null} you can return 
	 * the value from method {@link #unknownEventType()} to make your code more
	 * readable. 
	 * <p>
	 * Note:</br>
	 * If your event structure does not specify a unique id for each event type
	 * there is a convenient method that returns an {@link EventMetaData} instance 
	 * that maps id to name. Use it only for your known event types. See 
	 * {@link #equalNameAndIdEventMetaData(String)}.
	 * 
	 * @param eventName a string that may or may NOT correspond to a known event type in 
     ...
     * does not recognize the {@code eventName} in {@link #getEventMetaData(String)}
     * as a known event. Use this method instead of returning {@code null}, to make 
     ...
     * 
     * @param eventName name of a known event type.
